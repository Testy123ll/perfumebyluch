          ...
          <div className="mb-8 rounded-xl border border-border bg-card p-6 shadow-sm">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="mb-1 block text-sm">Image {editingId && !imageFile ? "(Leave blank to keep current)" : ""}</label>
                  <input
                    type="file"
                    accept="image/*"
              ...
              {loading && products.length === 0 ? (
                ...
                products.map((p) => (
                  <tr key={p.id} className="border-t border-border">
                    ...
                    <td className="p-4">
                      <span className={\`inline-flex rounded-full px-2 py-1 text-xs \${p.in_stock ? 'bg-green-500/10 text-green-500' : 'bg-red-500/10 text-red-500'}\`}>
                    ...
                    <td className="p-4 text-right">
                      <div className="flex justify-end gap-2">
                        <Button variant="ghost" size="icon" onClick={() => handleToggleVisibility(p)}>
                          {p.visible ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4 opacity-50" />}
                        </Button>
                        <Button variant="ghost" size="icon" onClick={() => handleEdit(p)}>
                          <Edit2 className="h-4 w-4" />
                        </Button>
                        <Button variant="ghost" size="icon" onClick={() => handleDelete(p.id)} className="text-red-500 hover:text-red-600">
